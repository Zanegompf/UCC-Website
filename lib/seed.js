 ...
 * never delete it. So that one is gated on this number and the number is
 * written, the same bargain the id migration makes.
 */
...
export const SEED = {
  ...
  /**
   * The share register starts empty rather than with invented holders: the
   * chief executive fills it in from the share page, and a made-up name on a
   * fresh record would read as a real holding. Equity is counted against
   * `stock.shares` above; votes have their own total here.
   */
  shareholders: { voterShares: 0, equity: [], voters: [] },
  ...
  legalTemplates: [],
  /**
   * Forum threads, each carrying its own replies. `board` names one of
   * FORUM_BOARDS, which is also what decides who may read it.
   */
  forum: [],
  /**
...
export async function ensureData() {
  ...
  if (data && data.company) {
    ...
    // answers an empty register for a missing one, so this is only so the key is
    // on the record once something saves.
    if (!data.shareholders) {
      data.shareholders = { voterShares: 0, equity: [], voters: [] };
    }

    // A thread typed in by hand in the control room has no `replies` array.
    // Back-fill rather than guarding every read of it.
    data.forum = data.forum.map((t) =>
      t && Array.isArray(t.replies) ? t : { ...t, replies: [] }
    );
     ...
     * added twice.
     */
    const stale = (data.version || 1) < RECORD_VERSION;
    const wantsResearch =
      stale &&
      (data.divisions || []).some((d) => d?.name === "Executive Committee") &&
