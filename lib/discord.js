 ...
 *
 * Posting a notice is the ONLY thing on the site that reaches a webhook, so in
 * practice these two are the same thing; the pair is kept so a hook can still
 * be aimed precisely and so a further event would have somewhere to go.
 *
 * "Client requests" and "Shift log" are deliberately absent. Both are filed
 * many times a day and name a person — a client and what they are buying, or a
 * worker and what they were paid for. A hook still storing either value matches
 * nothing, which is the intended outcome; it is NOT migrated onto a live event,
 * because that would start firing at a channel nobody aimed it at.
 */
export const HOOK_EVENTS = ["All posts", "Announcements"];
