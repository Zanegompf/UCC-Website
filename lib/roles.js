...
export function filterData(data, level) {
  ...
  /**
   * Retired features, stripped rather than simply no longer read.
   *
   * The forum, the financial record and the equity class of the share register
   * were removed from the site, but a record written while they existed still
   * carries them — nothing purges the stored blob, and a page save only
   * overwrites the keys it sends. Two of these were gated: the balance sheet
   * stopped at staff and a forum thread was gated by its board. Dropping those
   * gates while leaving the data on the record would have published both to
   * anyone who read /api/data. So they are deleted here, on the way out, which
   * is the only place that is true for every caller.
   */
  delete d.forum;
  delete d.financials;
  if (d.shareholders) delete d.shareholders.equity;
  ...
  // `shareholders` is never stripped. Who votes a listed company is the one
  // thing a register of voting shares is for, and the share page is the public
  // investor page. If it should ever become restricted, gate it here, because
  // doing it in Site.jsx would only hide what was still sent.
