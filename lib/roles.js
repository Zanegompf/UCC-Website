...
export function filterData(data, level) {
  ...
  if (level < LEVEL.staff) {
    ...
    d.shifts = [];
    // Applications carry someone's handle, pay expectations and references.
    // `jobs` deliberately stays public — the application form needs it to
    // render its dropdown for people who have not been hired yet.
    d.applications = [];
  }
