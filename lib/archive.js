 ...
 *
 * All of them are rows in a top-level list. A forum reply used to be the one
 * exception — it lived inside its thread, so the save-time diff could not see
 * it and it could not be restored by appending to a list of its own — and the
 * archive carried a second, nested code path for that alone. The forum is gone
 * and so is the path.
 */
export const ARCHIVE_KINDS = Object.keys(ARCHIVED_LISTS);

export function archiveLabel(kind) {
  return ARCHIVED_LISTS[kind] || kind;
}
