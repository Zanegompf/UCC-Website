 ...
 * client's request with it and nothing on the record remembered. These lists
 * now keep what was removed, on `deleted`, so an executive can at least read
 * back what used to be there.
 *
 * Not all of them, deliberately. The whole record is one blob that every page
 * load reads, so archiving `shifts` and `transactions` — the two that turn over
...
/**
 * Kinds that reach the archive without being a top-level list.
 *
 * A forum reply lives inside its thread, so the save-time diff cannot see it —
 * only the forum route knows a reply went, and it archives one explicitly. It
 * also cannot be restored by appending to a list of its own: it has to go back
 * into the thread it came from, which is what `parent`, `into` and `ref`
 * describe. `ref` names the field on the archived entry holding the parent's id.
 */
export const NESTED_ARCHIVED = {
  forumReply: {
    label: "Forum replies",
    parent: "forum",
    into: "replies",
...
/** Every kind that can appear on `deleted`, in the order the page shows them. */
export const ARCHIVE_KINDS = [
  ...Object.keys(ARCHIVED_LISTS),
  ...Object.keys(NESTED_ARCHIVED),
];

export function archiveLabel(kind) {
  return ARCHIVED_LISTS[kind] || NESTED_ARCHIVED[kind]?.label || kind;
}
